...
function generateSecureQR() {
  ...
  setTimeout(() => {
    ...
    if (password) {
      ...
      const encrypted = CryptoJS.AES.encrypt(JSON.stringify(dataObject), key, {
        iv,
      });
  ...
  reader.onload = function (e) {
    ...
    image.onload = function () {
      ...
      if (code) {
        ...
        qrRecognized = true;
        showMessage(
          "QR 인식 완료. 비밀번호가 있으면 입력 후 해석을 시도하세요."
        );
      } else {
        qrRecognized = false;
  ...
  setTimeout(() => {
    ...
    try {
      ...
      if (parsed.encryption) {
        ...
        if (
          realData.expiresIn &&
          Date.now() > realData.createdAt + realData.expiresIn
        ) {
        ...
        if (
          parsed.expiresIn &&
          Date.now() > parsed.createdAt + parsed.expiresIn
        ) {
          showMessage("⏰ QR 코드가 만료되었습니다.");
        } else {
      ...
      console.error(e);
      showMessage(
        "❌ 복호화 실패: 비밀번호가 틀렸거나 QR 내용이 잘못되었습니다."
      );
    }
    resetDecryptUI();
  }, 300);
